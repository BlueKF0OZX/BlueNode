...
class NodeSmartHandler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if action in ('node-connect', 'node-disconnect', 'node-switch', 'dodropin-connect', 'dodropin-disconnect'):
            ...
            if result.get('ok') and result.get('outcome') == 'verified':
                emit('CONTROL.' + action.replace('-', '.').upper(), result['message'])
            self.send_json(status, result)
            return
        ADMIN.audit('invalid-control', 'rejected')
        self.send_json(403, {'ok': False, 'error': 'Action not permitted'})
