...
def switch(payload, config):
    """Called under the control lock; affect only the two explicit targets."""
    local = str(config.get('node', ''))
    if (not isinstance(payload, dict) or set(payload) != {'node', 'from_node'}
            or not numeric(local) or not all(numeric(payload[k]) for k in ('node', 'from_node'))
            or len({local, payload['node'], payload['from_node']}) != 3):
        return 400, {'ok': False, 'error': 'Enter two different valid remote node numbers'}
    target, source = payload['node'], payload['from_node']
    for phase, action, node in [('disconnecting', 'node-disconnect', source),
                                 ('connecting', 'node-connect', target)]:
        code, result = perform_locked(action, {'node': node}, config)
        if not result.get('ok'):
            return code, dict(result, phase=phase, from_node=source, node=target,
                              error=f'Switch stopped while {phase} node {node}: ' + result['error'])
    final = asterisk_observation.node_evidence(local)
    links = final.get('links')
    if (final.get('status') == 'available' and asterisk_observation.fresh(final)
            and isinstance(links, list)
            and not any(link['node'] == source for link in links)
            and any(link['node'] == target and link['mode'] in ('T', 'R') for link in links)):
        return 200, {'ok': True, 'outcome': 'verified', 'phase': 'complete',
                     'node': target, 'from_node': source,
                     'message': f'Switched from node {source} to {target}; verified from App_Rpt state'}
    incident_id = uuid.uuid4().hex
    saved = record_failure({'id': incident_id, 'recorded_at': time.time(),
                            'action': 'node-switch', 'node': target, 'from_node': source,
                            'reason': 'switch_not_confirmed', 'last': snapshot(final)})
    return 504, {'ok': False, 'outcome': 'unverified', 'phase': 'verifying',
                 'node': target, 'from_node': source, 'reason': 'switch_not_confirmed',
                 'diagnostic_saved': saved, 'diagnostic_id': incident_id if saved else None,
                 'error': 'Final switch state could not be confirmed. Check current links before retrying'}


def perform_locked(action, payload, config):
